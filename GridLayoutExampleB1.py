...
def time():
    ...
    lbl_Clock_Label.after(1000, time)
lbl_Clock_Label = Label(mstr_grid, font=('calibri', 12, 'bold'))
lbl_Clock_Label.grid(row=1, column=1, columnspan=2, sticky='nw'); time()
...
def btn_button_click(yo):
    if checkvar.get(): mixer.music.load('YoYoYoBuddy.wav'); mixer.music.play()
